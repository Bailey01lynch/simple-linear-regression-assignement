...
epilepsy_dat %>%
  ...
  labs(title= "Ipsilateral HC Size vs. Years Since Diagnosis", x= "Years Since Diagnosis", y= "Ipsilateral HC Size")
```
...
epilepsy_dat %>%
  ...
  geom_point() +
  geom_smooth(method = "lm", se= FALSE) +
  labs(title= "Contralateral Hippocampus Size vs. Years Since Epilepsy Diagnosis", x= "Years Since Diagnosis", y= "Contralateral Hippocampus Size (cubic centimeters)")
```
...
timing_data %>%
  ...
  geom_boxplot() +
  labs(title= "Comparison of ProducedRate Across ExposureRate", x = "ExposureRate", y = "ProducedRate")
```
...
* The data is more consistent with the **priming** hypothesis were perception is *faster* after being exposed to fast rhythms.

Replace the ...s below with the correct values:
* The average (mean) rate produced by the participant in the fast condition is 396.925 ms.
